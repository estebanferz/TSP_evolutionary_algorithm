...
def run_single_configuration(config):
    ...
    for _ in range(cantidad_ejecuciones):
        ...
        ga_run = GeneticAlgorithm(
            population_size=tamanio_poblacion,
            generations=cantidad_de_generaciones,
            # Pass the instantiated operator objects directly
            selection_method=operador_seleccion,
            crossover_method=operador_cruce,
            crossover_rate=probabilidad_de_cruce,
            mutation_method=operador_mutacion,
            mutation_rate=probabilidad_de_mutacion,
            survivor_selector=operador_reemplazo,
            matrix=matriz_costos,
        )
        # --- ---

        tiempo_de_inicio = time.perf_counter()
        _, best_cost = ga_run.run(verbose=False) # Run this instance
...
if __name__ == '__main__':
    ...
    OPERADORES_SELECCION = [TournamentSelection(tournament_size=k) for k in TAMANIO_TORNEO] \
                         + [RouletteSelection()]
    OPERADORES_CRUCE = [PMXCrossover(), OXCrossover()]
    OPERADORES_MUTACION = [InversionMutation(), SwapMutation()]
    OPERADORES_REEMPLAZO = [ElitismSurvivorSelection(elitism_ratio=r) for r in PORCENTAJE_DE_REEMPLAZO] \
                         + [SteadyStateSurvivorSelection(replacement_rate=r) for r in PORCENTAJE_DE_REEMPLAZO_SS]
